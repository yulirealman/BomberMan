...
@export var think_time: float = 0.2                    # 放完炸弹或到达目标后的思考时间
...
#@export_range(0.0, 1.0) var aggro_chance: float = 0.6  # 60%概率优先追杀玩家，40%炸箱子
#@export var box_search_radius: int = 8                 # 找箱子的视野范围 (格)
#@export var think_time: float = 0.4                    # 思考停顿时间
#@export var bomb_drop_cooldown: float = 0.15           # 连环放炸弹的间隔 (实现7炸弹速刷)
#
#@export_group("节点引用 (References)")
#@export var bomb_scene: PackedScene              # 炸弹预制体
#
...
#func _physics_process(delta: float) -> void:
	...
	#match current_state:
		...
		#EnemyState.FLEE:
			## 逃跑时有概率随手扔雷封路
			#if _can_plant_bomb() and randf() < 0.03: 
				#_plant_bomb(my_grid_pos, false) 
				#
			#_move_along_path(delta)
			#if current_path.is_empty():
				#if DangerMap.is_cell_safe(my_grid_pos):
		...
		#EnemyState.CHASE_PLAYER, EnemyState.HUNT_BOX, EnemyState.WANDER:
			...
			#
			#if current_path.is_empty():
				#_plant_bomb(my_grid_pos, true)
				#
				## 🌟 贪婪速刷逻辑：手里还有炸弹且在炸箱子时，立刻寻找下一个目标，不发呆！
				#if current_state == EnemyState.HUNT_BOX and _can_plant_bomb():
					#_set_idle(0.05) 
				#else:
					#_set_idle(think_time)
#
...
#func _decide_next_action(current_grid: Vector2i) -> void:
	...
	#
	## 🌟 超视距狙击 (Sniping)
	#if _can_plant_bomb() and randf() < aggro_chance:
		#if _has_line_of_sight(current_grid, player_pos) and _manhattan_dist(current_grid, player_pos) <= bomb_power:
			#_plant_bomb(current_grid, false)
			#_start_fleeing(current_grid) 
			#return
			#
		#var sniper_pos = _find_sniper_position(current_grid, player_pos, 5) 
		#if sniper_pos != Vector2i(-1, -1):
			#current_path = _calculate_path(current_grid, sniper_pos)
			#if not current_path.is_empty():
				#current_state = EnemyState.CHASE_PLAYER
				#return
#
	## 🌟 十字路口爆破箱子
		...
		#var nearest_box = _find_box_in_radius(current_grid, box_search_radius)
		#if nearest_box != Vector2i(-1, -1):
			#var target_pad = _find_best_bomb_pad_for_box(current_grid, nearest_box)
			#if target_pad != Vector2i(-1, -1):
				#current_path = _calculate_path(current_grid, target_pad)
				#if not current_path.is_empty():
					#current_state = EnemyState.HUNT_BOX
...
#func _can_plant_bomb() -> bool:
	#return active_bombs.size() < max_bombs and bomb_cd_timer <= 0
#
#func _plant_bomb(grid_pos: Vector2i, force_pause: bool = true) -> void:
	#if not _can_plant_bomb(): return
	#if GridManager.has_bomb_at(grid_pos): return
	#
	## 【防自杀检查】死胡同绝不放炸弹
	#if _get_walkable_neighbors_count(grid_pos) <= 1: return 
#
	#var bomb = bomb_scene.instantiate()
	#bomb.global_position = GridManager.cell_to_world(grid_pos, GRID_SIZE)
	#
...
#func _has_line_of_sight(start: Vector2i, end: Vector2i) -> bool:
	...
	#return true
#
#func _find_sniper_position(start: Vector2i, target: Vector2i, max_walk: int) -> Vector2i:
	#var best_pos = Vector2i(-1, -1)
	#var min_dist = 999
	#for x in range(start.x - max_walk, start.x + max_walk + 1):
		#for y in range(start.y - max_walk, start.y + max_walk + 1):
			#var check_pos = Vector2i(x, y)
			#if not _is_in_bounds(check_pos) or not _is_cell_walkable(check_pos): continue
			#if _has_line_of_sight(check_pos, target) and _manhattan_dist(check_pos, target) <= bomb_power:
				#var dist_to_walk = _calculate_path(start, check_pos).size()
				#if dist_to_walk > 0 and dist_to_walk < min_dist:
					#if _get_walkable_neighbors_count(check_pos) >= 2:
						#min_dist = dist_to_walk
						#best_pos = check_pos
	#return best_pos
#
#func _find_best_bomb_pad_for_box(start: Vector2i, box_pos: Vector2i) -> Vector2i:
	#var best_pad = Vector2i(-1, -1)
	#var max_open_dirs = -1
	#var min_dist = 999
	#for dir in [Vector2i.UP, Vector2i.DOWN, Vector2i.LEFT, Vector2i.RIGHT]:
		#var pad = box_pos + dir
		#if _is_in_bounds(pad) and DangerMap.is_cell_safe(pad) and _is_cell_walkable(pad):
			#var walk_dist = _calculate_path(start, pad).size()
			#if walk_dist > 0 or start == pad:
				#var open_dirs = _get_walkable_neighbors_count(pad)
				#if open_dirs > max_open_dirs or (open_dirs == max_open_dirs and walk_dist < min_dist):
					#max_open_dirs = open_dirs
					#min_dist = walk_dist
					#best_pad = pad
	#return best_pad
#
